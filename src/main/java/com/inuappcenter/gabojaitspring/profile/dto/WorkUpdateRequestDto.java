...
public class WorkUpdateRequestDto {
    ...
    @ApiModelProperty(position = 1, required = true, dataType = "String", value = "경력 식별자")
    @NotBlank(message = "모든 필수 정보를 입력해주세요.", groups = ValidationSequence.NotBlank.class)
    private String workId;

    @ApiModelProperty(position = 2, required = true, dataType = "String", value = "기관명", example = "가보자잇회사")
    @NotBlank(message = "모든 필수 정보를 입력해주세요.", groups = ValidationSequence.NotBlank.class)
    ...
    @ApiModelProperty(position = 6, required = true, dataType = "String", value = "설명", example = "가보자잇에서 백엔드 개발")
