...
@Getter
@ToString
@ApiModel(value = "프로필 단건 조회 응답")
public class ProfileFindOtherResponse {
    ...
    @ApiModelProperty(position = 19, required = true, value = "생성일")
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    private LocalDateTime createdAt;

    @ApiModelProperty(position = 20, required = true, value = "수정일")
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    private LocalDateTime updatedAt;

    public ProfileFindOtherResponse(User user,
                                    List<Skill> skills,
                                    ProfileVO profile,
                                    List<Offer> offers,
                                    Boolean isFavorite) {
